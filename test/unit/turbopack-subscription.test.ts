...
describe('server HMR subscription recovery', () => {
  it('retries when the first subscription and its recovery both fail', async () => {
    ...
    const runSubscription = jest
      .fn<Promise<void>, []>()
      .mockRejectedValueOnce(subscriptionFailure)
  ...
  it('stops immediately when a failed recovery observes an abort', async () => {
    const controller = new AbortController()
    const runSubscription = jest.fn(async () => {
      throw new Error('subscription failed')
    })
    const recover = jest.fn(async () => {
      controller.abort()
      throw new Error('recovery failed after abort')
    })
