...
## Covering both states of an experiment: the experimental CI shards

CI runs the suites more than once — a plain run, plus numbered extra runs
marked by `__NEXT_EXPERIMENTAL_CI_SHARD`. (Today there is one shard, `1`, an
alias for the `--experimental` / `__NEXT_CACHE_COMPONENTS` run; see
`scripts/run-jest.sh`.) A fixture can key an experimental flag on a shard
number instead of pinning it — enabled by default, disabled in that shard:

```js
// next.config.js
const nextConfig = {
  cacheComponents: true, // pin every other dimension
  experimental: {
...
paired with a `// @gate useOffline` on the `describe`. A plain run — including
a local run with no special env — exercises the feature, while the shard run
asserts it is **inert** when disabled — and trips when the off state starts
passing. One suite, both states, no new CI job, and no duplicated fixture.
...
To reproduce the disabled (shard) state locally, set the marker the same way
CI does:

```sh
__NEXT_EXPERIMENTAL_CI_SHARD=1 NEXT_SKIP_ISOLATE=1 pnpm test-start-webpack test/e2e/app-dir/use-offline/use-offline.test.ts
```
